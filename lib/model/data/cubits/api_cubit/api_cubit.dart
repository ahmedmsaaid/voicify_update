...
import 'package:voicify/viewmodel/models/upload/save_url.dart';
...
class ApiCubit extends Cubit<ApiState> {
  ...
  Future<void> pickVideoFile() async {
    ...
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        try {
          await _convertAndProcessVideoFile(file, fileName);
        } on Exception catch (e) {
          throw Exception("لم يتم اختيار أي ملف. ${e.toString()}");
        }
        emit(SuccessPickVideoFile());
      }
    } catch (e) {
      emit(FailedPickVideoFile(msg: e.toString()));
      throw Exception("خطأ في اختيار ملف الفيديو: ${e.toString()}");
    }
  }
  ...
  Future<void> _convertAndProcessVideoFile(File file, String fileName) async {
    emit(LoadingConvertToMp3());
    try {
      String convertedPath = await convertVideoToMp3(file.path);
      await _processAudioFile(File(convertedPath), p.basename(convertedPath));
      emit(SuccessConvertToMp3());
    } catch (e) {
      emit(FailedConvertToMp3(msg: e.toString()));
  ...
  Future<String> convertVideoToMp3(String inputFilePath) async {
    final directory = await getApplicationDocumentsDirectory();
    final outputFileName = '${p.basenameWithoutExtension(inputFilePath)}.mp3';
    final outputFilePath = p.join(directory.path, outputFileName);
    try {
      await FFmpegKit.execute(
          '-i "$inputFilePath" -vn -ar 44100 -ac 2 -b:a 192k "$outputFilePath"');
      return outputFilePath;
    } catch (e) {
      print("خطأ أثناء تحويل الملف: $e");
      throw Exception("فشل تحويل الملف إلى MP3");
